...
const Navbar = ({ dark, setDark, colorTheme, setColorTheme }) => {
    ...
    return (
        <nav className="flex items-center justify-between h-20 px-6 md:px-16 bg-background text-textColor sticky top-0 z-40 bg-opacity-90">
            {/* Logo */}
            <div>
                <AnchorLink href="#" className="text-3xl font-bold text-textColor" onClick={closeMenu}>
                    <span className="text-primary">{data.logoLeft}</span>{data.logoRight}
                </AnchorLink>
            </div>

            {/* Hamburger button - mobile only */}
            <button onClick={toggleMenu} className="block md:hidden z-50" aria-label="Toggle menu">
